...
class RaceTrack {
  ...
  _spriteOval(ctx, runner, x, y, tx, ty) {
    ctx.save();
    ctx.translate(x, y);
    // Rotate so sprite faces direction of travel
    // drawAthlete draws upright (facing right). Oval tangent gives movement direction.
    // We want sprite "forward" to align with (tx,ty).
    // Sprite natural direction = +x (right). Rotate by angle of (tx,ty).
    const angle = Math.atan2(ty, tx);
    ctx.rotate(angle); // no offset needed — sprite faces right (+x)
    const scale = 0.68;
